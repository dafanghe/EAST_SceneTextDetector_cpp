...
float lanms::poly_iou(const Polygon &a, const Polygon &b) {
		cl::Clipper clpr;
...
void lanms::PolyMerger::add(const Polygon &p_given) {
		Polygon p;
		if (nr_polys > 0) {
...
lanms::PolyMerger::Polygon normalize_poly(const lanms::Polygon &ref, const lanms::Polygon &p) {
...
Polygon lanms::PolyMerger get() const {
		Polygon p;
...
std::vector<lanms::Polygon> lanms::standard_nms(std::vector<Polygon> &polys, float iou_threshold) {
		size_t n = polys.size();
		if (n == 0)
			return {};
...
std::vector<lanms::Polygon> lanms::merge_quadrangle_n9(const float *data, size_t n, float iou_threshold) {
			using cInt = cl::cInt;
